...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (receivedData == '\r')
	{
		...
		//__HAL_TIM_SET_COMPARE(&tim3, TIM_CHANNEL_1, tim3.Init.Period * DutyCycleServo(Data)/100);
...
static void MX_PID_Init(void)
{
	...
	pid.tau = 0.2;
	//Set max/min limit for anti integral windup

	//Set max/min system limit
