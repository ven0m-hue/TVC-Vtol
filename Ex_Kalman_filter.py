...
def measure_acc_mag(accel_data, mag_data):
    ...
    mx, my, mz = ( mag_data/np.linalg.norm(mag_data) ) * 0 # <-- mag not calibrated. Noisy data destroys the quaternion output
...
# This part needs to be modified while working with the rasberry PI 
# For now 
# RPY init
'''
from IMU   
...
# Sampling time using the ra pi
#def X_Kalman_filter(state_vec,P,Q,R):

#roll_offset, pitch_offset = self_calibrate_accel(N=1000) # accel calibration
#sleep(1)
...
def X_Kalman_filter(state_vec,P,Q,R):
    ...
    #new_state_vec = new_state_vec.reshape(7,1)
    #print(new_state_vec)
    ...
    # Update predicted error co-variance matrix 
    #P = (np.eye(7) - K.dot(H)).dot(P_next)
    ...
    estimate_roll, estimate_pitch = estimate_roll , estimate_pitch * 9/6.75  (# <--- Issue in the roll, perhaps the sensor issue) 
    ...
    #print('Roll = {.2f}, Pitch = {.2f}'.format( estimate_roll, estimate_pitch))
    #print('Angles[theta] : {0:.2f}, {1:.2f}'.format(np.around(estimate_roll, decimals=3), np.around(estimate_pitch, decimals=3)))
    #print(estimate_pitch)
    #print("*"*90)
    
    return estimate_roll, estimate_pitch
try:
